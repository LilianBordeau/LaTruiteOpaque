...
public class ControleurJeu  extends ControleurBase {
    ...
    private Couple<Integer,Integer> idToIndices(String id)
    {        
        ...
        return new Couple<>(ligne, colonne);
    }
    ...
    {        
        ...
        Couple<Integer,Integer> coordonnees = idToIndices(tuileGraphique.getId()); 
        if(navigation.moteur.jouer(coordonnees.premier, coordonnees.second))
        {            
            String idPingouin = indicesToId(coordonnees.premier, coordonnees.second, DEBUTIDPINGOUIN);        
            ImageView pingouinGraphique = (ImageView)anchorPane.getScene().lookup("#"+idPingouin);
            String nomImage = Constantes.nomImagePingouin(navigation.moteur.plateau.plateau[coordonnees.premier][coordonnees.second]);
